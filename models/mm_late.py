...
class MM_Model(nn.Module):
    def __init__(self, num_labels, txt_model_name, img_model_name, dropout, fusion_name='concat', device='cuda'):
...
class MMLate_Model(object):
    ...
    def __init__(self, config, txt_model_name, img_model_name, fusion_name, multilabel=False, device='cuda'):
        """ Initialization """
        ...
        if not self.cnn:
            ...
            self.model = MM_Model(
                ...
                txt_model_name=self.txt_model_name,
                img_model_name=self.img_model_name,
    ...
    te_filename=None):
        ...
        for epoch in range(epochs):
            ...
            metrics = compute_metrics(res_val_d, self.num_labels, multi_label=self.multilabel)
            metrics_dict = dict(zip(metrics["metric"], metrics["result"]))
            current_f1_macro = metrics_dict["f1_macro"]
            logger.info("Current val f1_macro: {:.4f}, Best val f1_macro: {:.4f}".format(current_f1_macro, best_f1_macro))
            ...
            if current_f1_macro > best_f1_macro:
                ...
                if model_path != None:
                    ...
                    logger.info("Best model saved with val_f1_macro: {:.4f}".format(best_f1_macro))
            else:
                patience_counter += 1
